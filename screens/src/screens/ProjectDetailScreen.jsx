...
const PROJECT = { name: 'App Launch Ideas', count: 14 }

function toTitleCase(str) {
  return str.replace(/\w\S*/g, w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase())
}

function headerFontSize(name) {
  if (name.length <= 12) return 30
  if (name.length <= 16) return 24
  return 20
}

const THIS_WEEK = [
  { descriptor: 'how to', title: 'validate a startup idea in 48 hours', duration: '9m', img: 'https://picsum.photos/seed/pd1/300/300' },
  { descriptor: 'the best', title: 'landing page frameworks for indie hackers', duration: '6m', img: 'https://picsum.photos/seed/pd2/300/300' },
  { descriptor: 'building', title: 'a waitlist that actually converts', duration: '14m', img: 'https://picsum.photos/seed/pd3/300/300' },
]

const LAST_WEEK = [
  { descriptor: 'why', title: 'most apps fail before launch', duration: '11m', img: 'https://picsum.photos/seed/pd4/300/300' },
  { descriptor: 'pricing', title: 'psychology for SaaS founders', duration: '7m', img: 'https://picsum.photos/seed/pd5/300/300' },
  { descriptor: 'how to', title: 'write copy that sells itself', duration: '5m', img: 'https://picsum.photos/seed/pd6/300/300' },
]
...
function BackArrow() {
  return (
    <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
      <path d="M14 4L7 11L14 18" stroke={C.PRIMARY} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
...
function PencilIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
      <path d="M15.5 3.5L18.5 6.5L8 17H5V14L15.5 3.5Z" stroke={C.PRIMARY} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  )
}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px 14px', background: C.BG }}>
      ...
      <span style={{
        ...
        fontSize, color: C.PRIMARY, fontWeight: 400,
        whiteSpace: 'nowrap', flex: 1, textAlign: 'center', padding: '0 12px',
      }}>
        {displayName}
      </span>
      ...
      {TABS.map(tab => {
        ...
        return (
          <div key={tab} style={{ flexShrink: 0 }}>
            ...
            {active && <div style={{ height: 2, background: C.ACCENT, marginTop: 4, borderRadius: 1 }} />}
          </div>
        )
      })}
    </div>
  ...
  return (
    <div style={{ width: 390, height: 844, background: C.BG, position: 'relative', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div className="no-scroll" style={{ flex: 1, overflowY: 'auto', paddingBottom: 100 }}>
        <SectionHeader label="This week" count="5" />
        <CardRow cards={THIS_WEEK} />
        <SectionHeader label="Last week" count="9" />
